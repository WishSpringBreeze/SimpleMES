...
def update_item_with_version(
    ...
    item_id: int, 
    item_data: SQLModel, # 这个 item_data 现在应该包含 version
    version_field: str = "version" # 版本字段的名称，默认为 'version'
) -> ModelType | None:
    """
    支持乐观锁的更新函数。
    """
    # 1. 从 item_data 中提取 version 值，并从要更新的数据中移除它
    #    因为 version 是用来做 WHERE 条件的，不是要 SET 的字段
    item_updates_dict = item_data.model_dump(exclude_unset=True)
    client_version = item_updates_dict.pop(version_field, None)

    if client_version is None:
        # 如果客户端没有提供 version，则无法执行乐观锁更新
        # 可以选择抛出异常或直接走普通更新逻辑
        # 这里我们选择抛出错误，强制使用乐观锁
        raise ValueError(f"The '{version_field}' field is required for updating.")

    # 2. 根据 ID 和 Version 构建查询和更新语句
    #    这是原子操作的关键
    db_item = session.get(model_class, item_id)
    if not db_item:
        return None # 没找到对象
    ...
    # 检查版本是否匹配
    current_db_version = getattr(db_item, version_field)
    if current_db_version != client_version:
        # 版本不匹配，说明数据已被修改
    ...
    setattr(db_item, version_field, new_version)
