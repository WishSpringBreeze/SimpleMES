...
@router.get("/by-name/{product_name}", response_model=ProductRead)
...
# # --- 新增：根据 Name 修改产品 ---
# @router.put("/by-name/{product_name}", response_model=ProductRead)
...
# --- 新增：根据 Name 删除产品 ---
@router.delete("/by-name/{product_name}", status_code=status.HTTP_204_NO_CONTENT)
...
def update_product(
    *, 
    ...
    """
    根据 ID 更新一个产品的信息 (使用乐观锁防止并发冲突)。
    """
    try:
        updated_product = update_item_with_version(session, Product, product_id, product_in)
    except ValueError as e:
        # 捕获到客户端未提供 version 的错误
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))

    if not updated_product:
        # update_item_with_version 返回 None 可能是因为没找到产品或版本冲突
        # 我们需要判断具体原因
        existing_product = get_item(session, Product, product_id)
        if not existing_product:
             raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Product not found")
        else:
             raise HTTPException(
                 status_code=status.HTTP_409_CONFLICT, 
                 detail=f"Product data conflict. The product was modified by another process. Please refresh and try again. Current version is {existing_product.version}."
             )
             
    return updated_product

# routers/products.py
# ... (@router.put("/by-name/{product_name}", ...) ...)
@router.put("/by-name/{product_name}", response_model=ProductRead)
def update_product_by_name(
    *, 
    session: Session = Depends(get_session), 
